...
import com.guusto.buygift.merchant.mapping.entity.MerchantAddressEntity_;
...
public class SearchMerchantAddress implements Specification {
	...
	public <T> Predicate toPredicate(CriteriaBuilder criteriaBuilder, CriteriaQuery<T> criteriaQuery, Root<?> root,
		...
		Predicate qPredicate = PredicateUtils.or(criteriaBuilder,
				PredicateUtils.like(q, expressionHandler.getExpression(MerchantAddressEntity_.CITY), criteriaBuilder),
				PredicateUtils.like(q, expressionHandler.getExpression(MerchantAddressEntity_.COUNTRY), criteriaBuilder),
				PredicateUtils.like(q, expressionHandler.getExpression(MerchantAddressEntity_.PROVINCE), criteriaBuilder));
