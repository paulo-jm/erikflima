...
export class DataSource<T, S> {
  ...
  constructor(private searchService: SearchService<T>, load = true) {

    if(load){
      this.search(this.specification, this.itemsPerPage, this.currentPage, this.ordersBy);
    }
  ...
  search(specification: S | string = null, itensPerPage: number = null, currentPage: number = null, ordersBy: Map<string, string> = null): void {

    this.loadingSubject.next(true);
    this.searchService.search(specification, itensPerPage, currentPage, ordersBy)
      .subscribe(
        result => {
          if (result) {
            this.totalRecordsSubject.next(result.totalRecords);
            this.numberOfRecordsSubject.next(result.numberOfRecords);
